...
// Dispatcher + Homepage OG template with live tool-count from the database.
// ═══════════════════════════════════════════════════════════════════════════════

function pt_render_awan_tools(array $p): string
{
    return match($p['template']) {
        'homepage_og' => pt_awan_tools_homepage_og($p),
        default       => pt_awan_tools_homepage_og($p),
    };
}

function pt_awan_tools_homepage_og(array $p): string
{
    $w    = 1200;
    $h    = 630;
    $font = PT_Text::e($p['font'] ?? 'Inter');

    // ── Live tool count from the plugins table ────────────────────────────────
    $tool_count = 500;
    try {
        if (class_exists('Database')) {
            $n = Database::getInstance()->count('plugins', "offered = 1");
            if ($n > 0) $tool_count = $n;
        }
    } catch (\Throwable $e) {}

    // Round down to nearest 10 (e.g. 524 → "520+"), minimum 10
    $nd = (string) max(10, (int)(floor($tool_count / 10) * 10));

    $heading = PT_Text::e($p['heading'] ?? 'Awan Tools');
    $footer  = PT_Text::e(strtoupper($p['footer'] ?? 'WWW.AWANTOOLS.SITE'));

    // ── Description lines (fixed structure, count injected) ───────────────────
    $d1 = htmlspecialchars("A complete library of {$nd}+ free online tools",  ENT_XML1);
    $d2 = "to simplify your digital life. Find utilities";
    $d3 = "for developers, productivity, and more.";
    $d4 = "Always free, no login required.";

    // ── Badge pills ───────────────────────────────────────────────────────────
    $bt1  = htmlspecialchars("{$nd}+ Tools",     ENT_XML1);
    $bt2  = "100% Free";
    $bt3  = "No Extra Logins";

    // Approximate width: ~10.2px per char + 44px padding, min 120
    $b1w  = max(120, (int)(mb_strlen($bt1) * 10.2 + 44));
    $b2w  = max(120, (int)(mb_strlen($bt2) * 10.2 + 44));
    $b3w  = max(120, (int)(mb_strlen($bt3) * 10.2 + 44));
    $bgap = 20;
    $bby  = 413;
    $bh   = 44;
    $brad = 22;
    $btot = $b1w + $b2w + $b3w + $bgap * 2;
    $bsx  = (int)(($w - $btot) / 2);
    $b1x  = $bsx;
    $b2x  = $bsx + $b1w + $bgap;
    $b3x  = $bsx + $b1w + $b2w + $bgap * 2;
    $b1cx = $b1x + (int)($b1w / 2);
    $b2cx = $b2x + (int)($b2w / 2);
    $b3cx = $b3x + (int)($b3w / 2);
    $bty  = $bby + 28;
    ...
    // ── Background dot grid ───────────────────────────────────────────────────
    $dots = '';
    for ($ddx = 30; $ddx <= $w; $ddx += 50) {
        for ($ddy = 30; $ddy <= $h; $ddy += 50) {
            $dots .= "<circle cx='$ddx' cy='$ddy' r='1.8' fill='#fff' opacity='0.13'/>";
        }
    }

    // ── Pre-computed coordinates ──────────────────────────────────────────────
    $cx      = 600;                 // horizontal center
    $lcy     = 88;                  // logo circle center-y
    $lr      = 48;                  // logo circle radius
    $lsr     = 54;                  // logo soft-ring radius
    $la_y    = $lcy + 1;            // "A" vertical center (dominant-baseline="middle")
    $title_y = 182;
    $d1y = 232;  $d2y = 272;  $d3y = 312;  $d4y = 352;
    $foot_y  = 592;

    // ── QR code pixel helpers ────────────────────────────────────────────────
    // Corner finder marker: outer white rect, inner blue rect, center white fill
    // Returns SVG rects for a QR corner marker at (ox,oy), square size 26
    // (using inline string since PHP functions can't be defined inside functions)
    $qr_ox1 = 1016; $qr_oy1 = 62;
    $qr_ox2 = 1062; $qr_oy2 = 62;
    $qr_ox3 = 1016; $qr_oy3 = 108;

    // Cloud body bottom rect coords
    $cl_rx = 1012; $cl_ry = 478; $cl_rw = 86; $cl_rh = 24;
    $cl_c1x = 1028; $cl_c1y = 478;
    $cl_c2x = 1052; $cl_c2y = 462;
    $cl_c3x = 1076; $cl_c3y = 474;

    // Sync arrow centers
    $sa_cx = 1054; $sa_cy = 473; $sa_r = 13;
    $sa_l1x = (int)($sa_cx - $sa_r); $sa_l2x = (int)($sa_cx + $sa_r);

    // PDF doc
    $pd_x = 1055; $pd_y = 248; $pd_w = 78; $pd_h = 88;
    $pd_cx = (int)($pd_x + $pd_w);                // right edge = fold corner x
    $pd_fold_pts = ($pd_x + $pd_w - 22) . ',' . $pd_y . ' ' .
                   ($pd_x + $pd_w) . ',' . ($pd_y + 22) . ' ' .
                   ($pd_x + $pd_w - 22) . ',' . ($pd_y + 22);
    $pd_lx = $pd_x + 8;
    $pd_label_cx = $pd_x + 22;
    $pd_label_y  = $pd_y + 18;
    $pd_mag_cx   = $pd_x + $pd_w + 14;
    $pd_mag_cy   = $pd_y + $pd_h + 8;
    $pd_mag_r    = 24;
    $pd_mag_ix   = $pd_x + $pd_w + 10;  // inner circle cx
    $pd_mag_iy   = $pd_y + $pd_h + 4;
    $pd_handle_x1 = $pd_x + $pd_w + 20;
    $pd_handle_y1 = $pd_y + $pd_h + 18;
    $pd_handle_x2 = $pd_x + $pd_w + 28;
    $pd_handle_y2 = $pd_y + $pd_h + 26;

    // Clipboard
    $cb_x = 44; $cb_y = 262; $cb_w = 106; $cb_h = 88;
    $cb_cx = (int)($cb_x + $cb_w / 2);
    $cb_tab_x = (int)($cb_cx - 23); $cb_tab_y = (int)($cb_y - 10);
    $cb_hole_x = (int)($cb_cx - 14); $cb_hole_y = (int)($cb_y - 14);
    $cb_inner_x = $cb_x + 10; $cb_inner_y = $cb_y + 14;
    $cb_inner_w = $cb_w - 20; $cb_inner_h = $cb_h - 20;
    $cb_t_y = (int)($cb_y + $cb_h * 0.68);

    // Image editor icon (bottom-left)
    $ie_x = 98; $ie_y = 434; $ie_w = 100; $ie_h = 88;
    $ie_cx = (int)($ie_x + $ie_w / 2);
    $ie_frame_x = $ie_x + 10; $ie_frame_y = $ie_y + 12;
    $ie_frame_w = $ie_w - 20; $ie_frame_h = $ie_h - 26;
    $ie_sky_h = (int)($ie_frame_h * 0.45);
    $ie_sun_cx = $ie_frame_x + $ie_frame_w - 10;
    $ie_sun_cy = $ie_frame_y + 10;
    // mountain 1 (left)
    $ie_m1b = $ie_frame_y + $ie_frame_h;  // bottom
    $ie_m1x1 = $ie_frame_x;
    $ie_m1xp = $ie_frame_x + 20;  // peak x
    $ie_m1yp = $ie_frame_y + (int)($ie_frame_h * 0.35);  // peak y
    $ie_m1x2 = $ie_frame_x + 46;
    // mountain 2 (right, overlaps)
    $ie_m2x1 = $ie_frame_x + 32;
    $ie_m2xp = $ie_frame_x + 56;
    $ie_m2yp = $ie_frame_y + (int)($ie_frame_h * 0.22);
    $ie_m2x2 = $ie_frame_x + $ie_frame_w;
    // brush position (diagonal, overlapping bottom-right of icon)
    $ie_brush_x1 = $ie_x + $ie_w - 4;  $ie_brush_y1 = $ie_y + $ie_h - 28;
    $ie_brush_x2 = $ie_x + $ie_w + 24; $ie_brush_y2 = $ie_y + $ie_h + 2;
...
<defs>
  ...
  <linearGradient id="bgGr" x1="0.15" y1="0" x2="0.85" y2="1">
    <stop offset="0%"   stop-color="#74c8f2"/>
    <stop offset="50%"  stop-color="#5db8e8"/>
    <stop offset="100%" stop-color="#4aa6d6"/>
  </linearGradient>
  ...
  <linearGradient id="orangeGr" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%"   stop-color="#fb923c"/>
    <stop offset="100%" stop-color="#c2410c"/>
  </linearGradient>
  <linearGradient id="yellowGr" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%"   stop-color="#fde68a"/>
    <stop offset="100%" stop-color="#f59e0b"/>
  </linearGradient>
  <linearGradient id="logoAGr" x1="0" y1="0" x2="0" y2="1">
    <stop offset="0%"   stop-color="#2563eb"/>
    <stop offset="100%" stop-color="#1e40af"/>
  </linearGradient>

  <!-- Filters -->
  <filter id="shdw" x="-25%" y="-25%" width="150%" height="150%">
    <feDropShadow dx="0" dy="5" stdDeviation="8" flood-color="#1a4875" flood-opacity="0.22"/>
  </filter>
  <filter id="shdw_sm" x="-20%" y="-20%" width="140%" height="140%">
    <feDropShadow dx="0" dy="3" stdDeviation="5" flood-color="#1a4875" flood-opacity="0.18"/>
  </filter>
  <filter id="glow_pink" x="-60%" y="-60%" width="220%" height="220%">
...
<g clip-path="url(#clip)">
  ...
  <!-- ═══ BACKGROUND ═══════════════════════════════════════════════════════ -->
  <rect width="$w" height="$h" fill="url(#bgGr)"/>
  $dots

  <!-- ═══ ICON 1: Code &lt;/&gt; — top-left, pink glow ═════════════════════ -->
  <ellipse cx="148" cy="106" rx="68" ry="52" fill="#f472b6" opacity="0.22" filter="url(#glow_pink)"/>
  <text x="148" y="130" text-anchor="middle"
        font-family="'Courier New',Courier,monospace"
        font-size="52" font-weight="900" fill="#db2777" letter-spacing="-2">&lt;/&gt;</text>

  <!-- ═══ ICON 2: QR Code — top-right, blue rounded square ════════════════ -->
  <g filter="url(#shdw_sm)">
    <rect x="1012" y="58" width="92" height="88" rx="16" fill="url(#blueGr)"/>
  </g>
  <!-- corner finder TL -->
  <rect x="$qr_ox1" y="$qr_oy1" width="26" height="26" rx="4" fill="white"/>
  <rect x="1019" y="65" width="20" height="20" rx="2.5" fill="#2563eb"/>
  <rect x="1022" y="68" width="14" height="14" fill="white"/>
  <!-- corner finder TR -->
  <rect x="$qr_ox2" y="$qr_oy2" width="26" height="26" rx="4" fill="white"/>
  <rect x="1065" y="65" width="20" height="20" rx="2.5" fill="#2563eb"/>
  <rect x="1068" y="68" width="14" height="14" fill="white"/>
  <!-- corner finder BL -->
  <rect x="$qr_ox3" y="$qr_oy3" width="26" height="26" rx="4" fill="white"/>
  <rect x="1019" y="111" width="20" height="20" rx="2.5" fill="#2563eb"/>
  <rect x="1022" y="114" width="14" height="14" fill="white"/>
  <!-- data pixels -->
  <rect x="1050" y="62" width="7" height="7" rx="1" fill="white"/>
  <rect x="1060" y="62" width="7" height="7" rx="1" fill="white"/>
  <rect x="1072" y="62" width="12" height="7" rx="1" fill="white"/>
  <rect x="1050" y="72" width="7" height="7" rx="1" fill="white"/>
  <rect x="1064" y="74" width="7" height="5" rx="1" fill="white"/>
  <rect x="1048" y="84" width="12" height="7" rx="1" fill="white"/>
  <rect x="1064" y="84" width="7" height="7" rx="1" fill="white"/>
  <rect x="1078" y="80" width="7" height="12" rx="1" fill="white"/>
  <rect x="1048" y="96" width="7" height="7" rx="1" fill="white"/>
  <rect x="1060" y="94" width="7" height="9" rx="1" fill="white"/>
  <rect x="1074" y="96" width="10" height="7" rx="1" fill="white"/>
  <rect x="1046" y="108" width="7" height="7" rx="1" fill="white"/>
  <rect x="1056" y="108" width="7" height="7" rx="1" fill="white"/>
  <rect x="1068" y="108" width="7" height="7" rx="1" fill="white"/>
  <rect x="1080" y="108" width="7" height="7" rx="1" fill="white"/>
  <rect x="1046" y="118" width="11" height="7" rx="1" fill="white"/>
  <rect x="1060" y="118" width="7" height="7" rx="1" fill="white"/>
  <rect x="1074" y="118" width="11" height="7" rx="1" fill="white"/>

  <!-- ═══ ICON 3: Clipboard + T — middle-left, green ════════════════════════ -->
  <g filter="url(#shdw_sm)">
    <rect x="$cb_x" y="$cb_y" width="$cb_w" height="$cb_h" rx="14" fill="url(#greenGr)"/>
  </g>
  <!-- top clip tab -->
  <rect x="$cb_tab_x" y="$cb_tab_y" width="46" height="18" rx="9" fill="#059669"/>
  <!-- hole/ring on tab -->
  <circle cx="$cb_cx" cy="$cb_tab_y" r="8" fill="#bbf7d0"/>
  <circle cx="$cb_cx" cy="$cb_tab_y" r="4" fill="#065f46"/>
  <!-- white paper area -->
  <rect x="$cb_inner_x" y="$cb_inner_y" width="$cb_inner_w" height="$cb_inner_h" rx="8" fill="white" fill-opacity="0.92"/>
  <!-- T letter -->
  <text x="$cb_cx" y="$cb_t_y" text-anchor="middle"
        font-family="Georgia,'Times New Roman',serif"
        font-size="40" font-weight="700" fill="#065f46">T</text>

  <!-- ═══ ICON 4: PDF + Magnifier — middle-right, yellow/orange ════════════ -->
  <!-- Yellow document -->
  <g filter="url(#shdw_sm)">
    <rect x="$pd_x" y="$pd_y" width="$pd_w" height="$pd_h" rx="10" fill="url(#yellowGr)"/>
  </g>
  <!-- folded corner (dark triangle) -->
  <polygon points="$pd_fold_pts" fill="#d97706"/>
  <!-- PDF label badge -->
  <rect x="$pd_lx" y="$pd_badge_y" width="40" height="18" rx="4" fill="#d97706"/>
  <text x="$pd_label_cx" y="$pd_label_y" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="10" font-weight="900" fill="white">PDF</text>
  <!-- text line stubs -->
  <rect x="$pd_lx" y="$pd_line1_y" width="46" height="5" rx="2.5" fill="#92400e" fill-opacity="0.45"/>
  <rect x="$pd_lx" y="$pd_line2_y" width="38" height="5" rx="2.5" fill="#92400e" fill-opacity="0.3"/>
  <rect x="$pd_lx" y="$pd_line3_y" width="42" height="5" rx="2.5" fill="#92400e" fill-opacity="0.3"/>
  <!-- Orange magnifier -->
  <g filter="url(#shdw_sm)">
    <circle cx="$pd_mag_cx" cy="$pd_mag_cy" r="$pd_mag_r" fill="url(#orangeGr)"/>
  </g>
  <circle cx="$pd_mag_ix" cy="$pd_mag_iy" r="15" fill="none" stroke="white" stroke-width="3.5"/>
  <line x1="$pd_handle_x1" y1="$pd_handle_y1" x2="$pd_handle_x2" y2="$pd_handle_y2"
        stroke="white" stroke-width="4" stroke-linecap="round"/>

  <!-- ═══ ICON 5: Image Editor — bottom-left, blue ════════════════════════════ -->
  <g filter="url(#shdw_sm)">
    <rect x="$ie_x" y="$ie_y" width="$ie_w" height="$ie_h" rx="14" fill="url(#blueGr)"/>
  </g>
  <!-- Image frame white border -->
  <rect x="$ie_frame_x" y="$ie_frame_y" width="$ie_frame_w" height="$ie_frame_h" rx="6" fill="white"/>
  <!-- Sky gradient top half -->
  <rect x="$ie_frame_x" y="$ie_frame_y" width="$ie_frame_w" height="$ie_sky_h" rx="6" fill="#bfdbfe"/>
  <!-- Sun -->
  <circle cx="$ie_sun_cx" cy="$ie_sun_cy" r="8" fill="#fbbf24"/>
  <!-- Mountain 1 (left, darker) -->
  <polygon points="$ie_m1x1,$ie_m1b $ie_m1xp,$ie_m1yp $ie_m1x2,$ie_m1b" fill="#60a5fa"/>
  <!-- Mountain 2 (right, lighter, tall) -->
  <polygon points="$ie_m2x1,$ie_m1b $ie_m2xp,$ie_m2yp $ie_m2x2,$ie_m1b" fill="#93c5fd"/>
  <!-- Ground strip -->
  <rect x="$ie_frame_x" y="$ie_m1b" width="$ie_frame_w" height="8" rx="0 0 6 6" fill="#bbf7d0"/>
  <!-- Paint brush (orange, diagonal, overlapping bottom-right) -->
  <g filter="url(#shdw_sm)">
    <rect x="-6" y="-4" width="38" height="11" rx="5.5" fill="#fb923c"
          transform="translate($ie_brush_x1,$ie_brush_y1) rotate(-38)"/>
    <rect x="10" y="0" width="12" height="24" rx="5" fill="#d97706"
          transform="translate($ie_brush_x1,$ie_brush_y1) rotate(-38)"/>
  </g>

  <!-- ═══ ICON 6: Cloud Sync — bottom-right, orange ═══════════════════════════ -->
  <g filter="url(#shdw)">
    <circle cx="$cl_c1x" cy="$cl_c1y" r="20" fill="url(#orangeGr)"/>
    <circle cx="$cl_c2x" cy="$cl_c2y" r="26" fill="url(#orangeGr)"/>
    <circle cx="$cl_c3x" cy="$cl_c3y" r="20" fill="url(#orangeGr)"/>
    <rect x="$cl_rx" y="$cl_ry" width="$cl_rw" height="$cl_rh" rx="10" fill="url(#orangeGr)"/>
  </g>
  <!-- Sync arrows (white) -->
  <path d="M$sa_l1x,$sa_cy A$sa_r,$sa_r 0 0,1 $sa_l2x,$sa_cy"
        fill="none" stroke="white" stroke-width="3.5" stroke-linecap="round"/>
  <polygon points="1067,466 1073,473 1061,473" fill="white"/>
  <path d="M$sa_l2x,$sa_cy A$sa_r,$sa_r 0 0,1 $sa_l1x,$sa_cy"
        fill="none" stroke="white" stroke-width="3.5" stroke-linecap="round"/>
  <polygon points="1041,480 1035,473 1047,473" fill="white"/>

  <!-- ═══ LOGO CIRCLE ══════════════════════════════════════════════════════ -->
  <!-- Soft halo ring -->
  <circle cx="$cx" cy="$lcy" r="$lsr" fill="white" fill-opacity="0.3" filter="url(#shdw)"/>
  <!-- Main white circle -->
  <circle cx="$cx" cy="$lcy" r="$lr" fill="white" filter="url(#shdw)"/>
  <!-- "A" letter with blue gradient -->
  <text x="$cx" y="$la_y" text-anchor="middle" dominant-baseline="middle"
        font-family="'$font',sans-serif"
        font-size="46" font-weight="900" fill="url(#logoAGr)">A</text>

  <!-- ═══ HEADING ══════════════════════════════════════════════════════════ -->
  <text x="$cx" y="$title_y" text-anchor="middle"
        font-family="'$font',sans-serif"
        font-size="58" font-weight="800" fill="#0a1628" letter-spacing="-0.5">$heading</text>

  <!-- ═══ DESCRIPTION ══════════════════════════════════════════════════════ -->
  <text x="$cx" y="$d1y" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="26" fill="#0d1f35">$d1</text>
  <text x="$cx" y="$d2y" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="26" fill="#0d1f35">$d2</text>
  <text x="$cx" y="$d3y" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="26" fill="#0d1f35">$d3</text>
  <text x="$cx" y="$d4y" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="26" fill="#0d1f35">$d4</text>

  <!-- ═══ BADGE PILLS ═══════════════════════════════════════════════════════ -->
  <rect x="$b1x" y="$bby" width="$b1w" height="$bh" rx="$brad"
        fill="white" fill-opacity="0.28" stroke="white" stroke-opacity="0.35" stroke-width="1"/>
  <text x="$b1cx" y="$bty" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="18" font-weight="600" fill="#0a1628">$bt1</text>

  <rect x="$b2x" y="$bby" width="$b2w" height="$bh" rx="$brad"
        fill="white" fill-opacity="0.28" stroke="white" stroke-opacity="0.35" stroke-width="1"/>
  <text x="$b2cx" y="$bty" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="18" font-weight="600" fill="#0a1628">$bt2</text>

  <rect x="$b3x" y="$bby" width="$b3w" height="$bh" rx="$brad"
        fill="white" fill-opacity="0.28" stroke="white" stroke-opacity="0.35" stroke-width="1"/>
  <text x="$b3cx" y="$bty" text-anchor="middle"
        font-family="'$font',sans-serif" font-size="18" font-weight="600" fill="#0a1628">$bt3</text>

  <!-- ═══ FOOTER URL ════════════════════════════════════════════════════════ -->
  <text x="$cx" y="$foot_y" text-anchor="middle"
        font-family="'$font',sans-serif"
        font-size="17" font-weight="700" letter-spacing="3" fill="#0d2847" fill-opacity="0.85">$footer</text>
